...
class Config:
    ...
    AI_MODEL = os.getenv("AI_MODEL", "llama-3.3-70b-instruct")
    ...
    def validate(cls):
        ...
        try:
            ...
            logger.warning("Invalid REDIS_SENTINELS format. Using defaults.")
            cls.REDIS_SENTINELS = ["localhost:26379"]
...
def create_database_engines():
    ...
    try:
        if not config.DATABASE_URL:
            ...
            database_url = "sqlite+aiosqlite:///./navigator.db"
        else:
...
class RedisManager:
    ...
    async def initialize(self):
        ...
        try:
            sentinel_hosts = []
            for sentinel in config.REDIS_SENTINELS:
                try:
                    host, port = sentinel.strip().split(':')
                    sentinel_hosts.append((host, int(port)))
                except ValueError:
                    logger.warning(f"Invalid sentinel format: {sentinel}")
                    continue
            
            if not sentinel_hosts:
                logger.warning("No valid sentinels configured, using direct Redis connection")
                # Fallback to direct Redis connection
                import redis.asyncio as aioredis
                self.master = aioredis.from_url("redis://localhost:6379", decode_responses=True)
            ...
            logger.warning(f"Redis Sentinel initialization failed: {e}. Using fallback storage.")
            self._initialized = False
...
celery_app.conf.update(
    ...
    enable_utc=True,
    task_routes={
        'generate_prompt_task': {'queue': 'generation'},
        'evaluate_prompt_task': {'queue': 'evaluation'},
        '*_prompt_task': {'queue': 'genertion'}
    },
...
class EvaluatePromptRequest(BaseModel):
    ...
    @field_validator('prompt', 'user_needs')
    @classmethod
...
@celery_app.task(bind=True, max_retries=3, name='generate_prompt_task')
...
@celery_app.task(bind=True, max_retries=3, name='evaluate_prompt_task')
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Don't raise to allow partial functionality
...
async def get_user_prompts(
    ...
    try:
        ...
        return [
            PromptResponse(
                id=row["id"],
                text=decrypt_sensitive_data(row["text"]) if row["text"] else "",
...
async def clear_cache(
    ...
    try:
        ...
        if redis_manager._initialized and redis_manager.master:
            await asyncio.get_event_loop().run_in_executor(
...
@app.post("/prompts/evaluate/stream")
async def evaluate_prompt_stream(eval_request: EvaluatePromptRequest, current_user: dict = Depends(verify_token)):
    """Stream evaluation results as they complete"""
    async def generate():
        try:
            yield f"data: {json.dumps({'status': 'starting', 'step': 'initializing'})}\n\n"
            
            university_config = await get_university_config(current_user["university_code"])
            yield f"data: {json.dumps({'status': 'progress', 'step': 'university_config_loaded'})}\n\n"
            ...
            yield f"data: {json.dumps({'status': 'progress', 'step': 'evaluation_queued', 'task_id': task.id})}\n\n"
            ...
            while True:
                ...
                if result.ready():
                    if result.successful():
                        task_result = result.result
                        yield f"data: {json.dumps({'status': 'completed', 'results': task_result})}\n\n"
                        break
                    else:
                        yield f"data: {json.dumps({'status': 'error', 'message': str(result.result)})}\n\n"
                        break
                else:
                    yield f"data: {json.dumps({'status': 'progress', 'step': 'evaluating'})}\n\n"
...
async def list_public_universities():
    """List available universities (public endpoint)"""
    try:
        async with AsyncSessionLocal() as db:
            query = text("SELECT code, name, domain FROM universities WHERE is_active = true ORDER BY name")
            result = await db.execute(query)
            universities = result.mappings().fetchall()
        ...
        logger.error(f"List public universities failed: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve universities"
        )
...
def generate_config():
    ...
    logger.info("Configuration generated in .env.generated")
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "__main__:app",  # Use __main__ for direct execution
        host=host,
        port=port,
